...
async function main(gpu: GPU) {
    ...
    render_pass_encoder0022.setViewport(0, 0, texture000.width / 2, texture000.height / 2, 0, 1);
    ...
    buffer008.destroy()
    render_pass_encoder0023.setScissorRect(0, 0, texture000.width / 2, texture000.height / 2);
    render_pass_encoder1001.setBlendConstant([1.0, 0.0, 0.0, 1.0]);
    device10.popErrorScope().then((error) => {
        if (error) {
            console.error(`An error occurred: ${error.message}`);
    ...
    device00.queue.writeBuffer(buffer0036, 0, uint32_0011, 0, uint32_0011.length);

    compute_pass_encoder0011.dispatchWorkgroupsIndirect(buffer0036, 0);
    compute_pass_encoder1030.end();
    compute_pass_encoder0010.end();
    compute_pass_encoder1030.dispatchWorkgroups(100);
