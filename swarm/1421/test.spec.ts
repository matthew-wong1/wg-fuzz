...
async function main(gpu: GPU) {
    ...
    {
        await buffer903.mapAsync(
            GPUMapMode.READ,
            0,
            400,
        );
        
        const copyArrayBuffer = buffer903.getMappedRange(0, 400);
        const data = copyArrayBuffer.slice(0);
        buffer903.unmap();
        console.log(new Float32Array(data));
    }
    ...
    try {
        shader_module1802_code = await fs.readFile('/Users/matthew/Documents/msc/final_proj/dawn_coverage_final/third_party/webgpu-cts/src/webgpu/api/operation/wg_fuzz/shader_module1802.wgsl', 'utf8');
    } catch (err) {
        console.log(err);
    }
    ...
    const render_bundle_encoder1901 = device190.createRenderBundleEncoder({
        label: "render_bundle_encoder1901",
        colorFormats: ["bgra8unorm"]
    });
