// {"0:0":[100,255,141,134,117,7,155,149,218,126,66,224,88,201,18,215,120,176,57,139,9,252,230,61,75,206,146,149,99,110,201,245,86,212,208,64,239,245,48,14,161,120,244,113,52,30,56,166]}
// Seed: 4818717195629802196
...
fn func_3(arg_0: bool, arg_1: vec2<bool>, arg_2: Struct_2, arg_3: bool) -> bool {
    ...
    var var_1 = max(~(~(~u_input.c)) % max(~u_input.c, ~(u_input.c | vec2<u32>(0u, 13240u))), select(vec2<u32>(u_input.c.x, 0u), ~(u_input.c % u_input.c), arg_0 != any(vec3<bool>(true, arg_0, true))));
    return -arg_2.b > firstTrailingBit(2147483647i ^ 25790i);
}
...
fn func_2() -> vec4<u32> {
    ...
    global3 = array<Struct_2, 30>();
    var var_2 = 19785u;
    var var_3 = Struct_1(abs(~u_input.c.x), !vec4<bool>(!(true & false), true || !false, false, func_3(!false, !vec2<bool>(global0[u_input.a], global0[u_input.c.x]), global3[1u << u_input.a], true & global0[u_input.c.x])), u_input.b.x, u_input.b.zz, -(-u_input.b.zyw));
    return (countOneBits(~vec4<u32>(u_input.c.x, 34976u, 1u, var_3.a)) ^ (vec4<u32>(~0u, 4294967295u, dot(vec4<u32>(2543u, var_3.a, 1u, 19058u), vec4<u32>(0u, 1u, var_3.a, var_3.a)), 94525u % 50589u) * vec4<u32>(u_input.c.x, var_3.a, countOneBits(var_3.a), 68499u ^ 6433u))) & vec4<u32>(var_3.a | u_input.a, select(reverseBits(4294967295u), ~1u, global0[select(dot(vec4<u32>(u_input.a, u_input.c.x, 1u, 0u), vec4<u32>(u_input.c.x, var_3.a, var_3.a, 0u)), firstTrailingBit(var_3.a), u_input.c.x > u_input.a)]), ~(u_input.a + 30374u), u_input.a);
}

fn func_4(arg_0: i32, arg_1: vec4<u32>, arg_2: f32, arg_3: vec3<bool>) -> bool {
    global2 = array<Struct_2, 9>();
    let var_0 = sign(-(-vec3<f32>(-arg_2, arg_2, arg_2 * 1000f)));
    let var_1 = (firstLeadingBit((u_input.a * 60727u) ^ arg_1.x) / 7198u) | 4294967295u;
    global3 = array<Struct_2, 30>();
    global2 = array<Struct_2, 9>();
    return !((arg_2 / arg_2) != -430f);
}
...
fn func_1(arg_0: Struct_2, arg_1: u32, arg_2: vec2<u32>, arg_3: Struct_2) -> bool {
    ...
    var_0 = arg_0.c.b;
    var var_1 = arg_3.d;
    var_0 = vec4<bool>(!false, all(vec2<bool>(func_4(var_1.e.x, func_2(), -1000f, select(var_0.yyx, vec3<bool>(false, arg_3.a.b.x, arg_3.c.b.x), true)), -2057f > -(-882f))), all(!(!vec2<bool>(var_0.x, var_1.b.x))), var_1.b.x);
    let var_2 = arg_3.a;
...
fn func_5(arg_0: bool, arg_1: vec2<f32>, arg_2: Struct_1) -> StorageBuffer {
    let var_0 = global3[1u];
    let var_1 = global2[~(~abs(u_input.c.x - 0u)) * ~arg_2.a];
    return StorageBuffer((17125u / var_1.a.a) << (~(~u_input.c.x) << (abs(47845u) / ~1u)), u_input.c, ~var_1.c.c, vec3<i32>(reverseBits(~(-2147483647i)), -var_0.a.d.x, reverseBits(u_input.b.x / 18783i) % (19869i / abs(-43764i))), sign(203f));
}
...
fn main() {
    ...
    let var_0 = 282i;
    global3 = array<Struct_2, 30>();
    let var_1 = vec2<bool>(all(!vec2<bool>(!global0[u_input.c.x], true)), false);
    global2 = array<Struct_2, 9>();
    global2 = array<Struct_2, 9>();
    let x = u_input.a;
    s_output = func_5(true, vec2<f32>((abs(-296f) + (293f * 1670f)) - select(-2189f, 2427f, func_1(Struct_2(Struct_1(u_input.c.x, vec4<bool>(false, global0[1u], var_1.x, false), var_0, vec2<i32>(33004i, var_0), u_input.b.xzy), var_0, Struct_1(u_input.a, vec4<bool>(var_1.x, true, false, true), 0i, u_input.b.wx, vec3<i32>(33577i, 2147483647i, 0i)), Struct_1(u_input.c.x, vec4<bool>(false, global0[u_input.c.x], global0[u_input.a], var_1.x), 99112i, u_input.b.wy, vec3<i32>(1i, 1i, i32(-2147483648))), 36375u), u_input.a, u_input.c, global3[4294967295u])), 166f + -361f), Struct_1(~(~119181u) ^ ~(~u_input.a), !select(select(vec4<bool>(false, var_1.x, global0[u_input.a], false), vec4<bool>(true, false, var_1.x, var_1.x), false), !vec4<bool>(global0[0u], var_1.x, true, false), !vec4<bool>(false, global0[4294967295u], var_1.x, var_1.x)), ~(-reverseBits(0i)), u_input.b.xw, u_input.b.zyx));
}
