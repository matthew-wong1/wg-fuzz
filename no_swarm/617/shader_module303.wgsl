...
fn func_3(arg_0: Struct_4, arg_1: vec2<bool>, arg_2: vec2<u32>, arg_3: f32) -> u32 {
    ...
    let var_1 = select(abs(firstTrailingBit(vec2<u32>(arg_2.x, 20605u))) | vec2<u32>(~3312u - ~4294967295u, ~abs(48233u)), min(abs(firstTrailingBit(arg_0.e.b)), vec2<u32>(1050u, (arg_0.e.c >> 4294967295u) >> arg_2.x)), arg_1.x);
    let var_2 = arg_0.d;
    let var_3 = -53805i != (~global1.x ^ -firstTrailingBit(u_input.a));
    var var_4 = vec3<u32>(~dot(vec2<u32>(countOneBits(var_2.b.x), countOneBits(26984u)), ~vec2<u32>(62526u, 0u)), firstLeadingBit(var_2.c), ~(4294967295u + 3605u));
    return ~(var_4.x >> reverseBits(~abs(15203u)));
}

fn func_2() -> f32 {
    var var_0 = 13388u;
    let var_1 = abs(exp2(-683f));
    let var_2 = countOneBits(u_input.a) << (func_3(Struct_4(global0[114501u], Struct_3(Struct_2(vec3<i32>(global1.x, -30421i, global1.x), Struct_1(vec4<i32>(u_input.a, 12112i, 0i, -28957i), vec2<u32>(41139u, 0u), 25961u, vec4<i32>(global1.x, 2147483647i, -21696i, global1.x)), global2[43908u], -160f), Struct_2(vec3<i32>(global1.x, global1.x, global1.x), Struct_1(vec4<i32>(-16386i, u_input.a, -9635i, u_input.a), vec2<u32>(4294967295u, 57834u), 0u, vec4<i32>(u_input.a, 2147483647i, u_input.a, -20458i)), false, -158f)), Struct_2(vec3<i32>(1i, u_input.a, -6610i), Struct_1(vec4<i32>(global1.x, global1.x, global1.x, global1.x), vec2<u32>(4294967295u, 13289u), 0u, vec4<i32>(-1i, -54283i, global1.x, -1i)), global2[4294967295u], var_1), Struct_1(vec4<i32>(72129i, -32348i, global1.x, 59522i), vec2<u32>(4294967295u, 46267u), 29788u, vec4<i32>(1i, 2147483647i, 1i, 0i)), Struct_1(vec4<i32>(i32(-2147483648), 33037i, global1.x, u_input.a), vec2<u32>(40766u, 4294967295u), 0u, vec4<i32>(u_input.a, -28081i, i32(-2147483648), -39497i))), !vec2<bool>(global2[1u], true), ~vec2<u32>(73017u, 39889u) % (vec2<u32>(10233u, 36129u) | vec2<u32>(1u, 66203u)), -1449f) * ~(~0u));
    var var_3 = (global2[dot(max(min(vec3<u32>(33894u, 19551u, 87674u), vec3<u32>(1u, 18995u, 57904u)), select(vec3<u32>(1u, 1u, 51437u), vec3<u32>(10641u, 31598u, 68555u), vec3<bool>(false, true, true))), max(countOneBits(vec3<u32>(4294967295u, 4294967295u, 2971u)), vec3<u32>(1245u, 4294967295u, 23125u) << vec3<u32>(30921u, 41291u, 1u)))] == !(!true)) & !global2[(~0u ^ abs(37906u)) ^ (50400u + 1u)];
    global1 = ~(vec3<i32>(global1.x, u_input.a, select(~u_input.a, -54043i | var_2, !true)) ^ reverseBits(select(vec3<i32>(var_2, -17604i, u_input.a), vec3<i32>(-34983i, u_input.a, -44713i), global2[1u]) & -vec3<i32>(-1i, global1.x, -25112i)));
    return 518f;
}

fn func_1() -> Struct_2 {
    let var_0 = vec2<f32>(-(-(-138f)), -1528f) * vec2<f32>(step((-549f / -1000f) + func_2(), -1000f / -447f), -step(-2110f, trunc(1190f)));
    let var_1 = Struct_2(~(-(-vec3<i32>(global1.x, global1.x, 0i))) | ~vec3<i32>(min(u_input.a, 58923i), ~1i, countOneBits(-1i)), Struct_1(-(~vec4<i32>(global1.x, 24079i, -7109i, global1.x) >> vec4<u32>(0u, 49412u, 0u, 24426u)), abs(select(vec2<u32>(4294967295u, 17513u) % vec2<u32>(26438u, 60044u), ~vec2<u32>(4294967295u, 0u), !vec2<bool>(global2[32023u], global2[1u]))), ((14909u * 67915u) - dot(vec3<u32>(4294967295u, 39579u, 4294967295u), vec3<u32>(23763u, 51078u, 0u))) + 32748u, select(abs(vec4<i32>(global1.x, global1.x, u_input.a, 31435i)), select(~vec4<i32>(-1i, -14887i, 96900i, 14021i), vec4<i32>(-16302i, global1.x, 1i, 1i), false), vec4<bool>(global2[~30067u], !true, !true, any(vec2<bool>(global2[0u], global2[1u]))))), -(-var_0.x + sign(806f)) == 1974f, var_0.x);
    var var_2 = any(select(select(!select(vec2<bool>(global2[56377u], var_1.c), vec2<bool>(global2[76415u], false), global2[var_1.b.c]), !(!vec2<bool>(var_1.c, global2[var_1.b.c])), false), select(select(!vec2<bool>(false, false), select(vec2<bool>(var_1.c, var_1.c), vec2<bool>(false, false), vec2<bool>(global2[var_1.b.b.x], true)), !vec2<bool>(global2[0u], true)), vec2<bool>(var_1.c, any(vec2<bool>(var_1.c, false))), !var_1.c), select(vec2<bool>(false, any(vec3<bool>(true, true, global2[var_1.b.b.x]))), select(vec2<bool>(global2[5722u], global2[32430u]), vec2<bool>(global2[1u], var_1.c), var_1.c | global2[var_1.b.c]), vec2<bool>(true, true == var_1.c))));
...
fn main() {
    ...
    var var_1 = func_1().b;
    let var_2 = Struct_4(global0[0u], var_0, Struct_2(var_1.d.yxx, func_1().b, all(!vec3<bool>(false, global2[var_1.c], global2[var_0.a.b.b.x])), step(var_0.a.d, -(-1715f)) - (-1719f / max(var_0.b.d, var_0.b.d))), Struct_1(vec4<i32>(dot(vec3<i32>(var_0.b.b.d.x, u_input.a, -47945i), vec3<i32>(global1.x, u_input.a, u_input.a)), var_0.b.a.x, 0i, -941i / -1i) % vec4<i32>(dot(vec4<i32>(3206i, var_0.b.b.a.x, -8829i, var_0.a.b.a.x), vec4<i32>(-1i, global1.x, 63821i, var_1.d.x)), var_0.a.b.d.x, var_1.d.x + -3300i, u_input.a), var_0.b.b.b, 93260u % firstLeadingBit(~46471u), vec4<i32>(global1.x | -var_1.a.x, dot(var_1.a, vec4<i32>(var_0.b.b.d.x, global1.x, global1.x, u_input.a) & vec4<i32>(var_0.b.b.a.x, -24371i, var_1.a.x, global1.x)), -1i, -u_input.a << reverseBits(2772u))), var_0.b.b);
    var_1 = var_2.a.a.b;
    let var_3 = func_1();
    let x = u_input.a;
    s_output = StorageBuffer(-max(vec3<f32>(-410f * -139f, var_2.a.a.d, 1342f * var_3.d), round(step(vec3<f32>(545f, var_2.c.d, var_2.a.b.d), vec3<f32>(var_3.d, var_2.a.a.d, var_3.d)))));
}
