// {"0:0":[215,192,141,151,227,104,37,55,70,220,136,245]}
// Seed: 11191097839611924602
...
fn func_2(arg_0: vec3<i32>, arg_1: Struct_4, arg_2: u32, arg_3: vec3<u32>) -> u32 {
    var var_0 = Struct_2(-vec2<f32>(min(-1368f / arg_1.b.c.a.x, -449f + -1365f), arg_1.a.a.x), -arg_0.x <= 19247i);
    return 4294967295u;
}

fn func_3() -> i32 {
    let var_0 = vec3<i32>(~(~1i) + -14342i, -1i, 1i - abs(4678i));
    global2 = array<vec3<f32>, 23>();
    var var_1 = 787f;
    var var_2 = true;
    var var_3 = global1[u_input.c];
    return 1i;
}
...
@workgroup_size(1)
fn main() {
    var var_0 = step(-(vec4<f32>(-194f, 565f, -1029f - -402f, func_1(false, Struct_1(vec3<i32>(-1i, -1i, 2147483647i), vec4<f32>(108f, -1697f, 381f, -2557f), vec3<i32>(-1i, 1i, 2147483647i)))) * ((vec4<f32>(259f, -1095f, -699f, 876f) - vec4<f32>(3154f, -1294f, -921f, 1418f)) / vec4<f32>(-1321f, 1469f, -1388f, 874f))), step(select(step(vec4<f32>(605f, 551f, -122f, -975f) * vec4<f32>(-280f, -850f, 2361f, -1000f), max(vec4<f32>(698f, -1000f, -623f, -234f), vec4<f32>(814f, -1000f, -975f, 153f))), exp2(round(vec4<f32>(-670f, 646f, 1617f, 541f))), select(!vec4<bool>(false, true, false, false), select(vec4<bool>(true, false, true, true), vec4<bool>(false, false, false, true), vec4<bool>(true, false, false, true)), vec4<bool>(false, true, true, true))), vec4<f32>(947f / 1580f, 2698f, -1000f * -254f, floor(961f)) + vec4<f32>(1661f + -818f, 1708f, -682f, 459f * -774f)));
    var var_1 = ~(~vec3<u32>(107416u, (34896u & u_input.a) % u_input.c, ~(1u / 1u)));
    let var_2 = min(vec3<i32>(~1i, -9698i, ((1i & -1i) ^ 13442i) << ~func_2(vec3<i32>(i32(-2147483648), 32637i, 16240i), Struct_4(global1[4294967295u], Struct_3(vec2<f32>(var_0.x, -1115f), var_1.xx, Struct_2(var_0.xz, true), global1[var_1.x], vec4<bool>(false, true, false, false)), true), 84156u, vec3<u32>(0u, u_input.a, u_input.c))), vec3<i32>(47433i, 16240i * (clamp(-9792i, 1i, 1i) ^ 6900i), -(func_3() + -29011i)));
    var var_3 = var_2.yz;
    let var_4 = ~((var_1.x / min(~u_input.c, abs(u_input.c))) ^ var_1.x);
    var var_5 = false;
    let x = u_input.a;
    s_output = StorageBuffer(((~vec3<u32>(var_1.x, var_1.x, 4294967295u) >> (vec3<u32>(var_4, var_4, var_1.x) * vec3<u32>(u_input.c, var_1.x, u_input.a))) >> select(~vec3<u32>(36000u, u_input.c, var_1.x), vec3<u32>(var_1.x, u_input.b, 1u), select(vec3<bool>(false, true, true), vec3<bool>(false, false, false), vec3<bool>(false, true, true)))) % ~(vec3<u32>(1u, 32662u, u_input.a) + countOneBits(vec3<u32>(80911u, u_input.c, 41152u))), ~vec4<i32>(var_3.x, ~var_2.x, -abs(0i), ~var_2.x), 46618u);
}
